...
def test_epoch(data_generator, model, criterion, dcase_output_folder, params, device):
    ...
    with torch.no_grad():
        for data, target in data_generator.generate():
            ...
            data, target = torch.tensor(data).to(device).cfloat(), torch.tensor(target).to(device).float()
            ...
            # print("test_filelist[file_cnt]!!!!", file_cnt, dcase_output_folder, test_filelist)
            ...
            test_loss += loss.item()
            nb_test_batches += 1
            if params['quick_test'] and nb_test_batches == 1:
                break
...
def train_epoch(data_generator, optimizer, model, criterion, params, device):
    ...
    for data, target in data_generator.generate():
        ...
        nb_train_batches += 1
        if params['quick_test'] and nb_train_batches == 1:
            break
...
def main(argv):
    ...
    if params['mode'] == 'dev':
        ...
        # elif 'FSD50K' in params['dataset_dir'] or 'SPEECH' in params['dataset_dir']:
        #     print("Falling into this logic")
        #     test_splits = [[3]]
        #     val_splits = [[2]]
        #     train_splits = [[1]]
