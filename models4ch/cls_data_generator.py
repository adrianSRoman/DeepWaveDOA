        ...
        if self._is_eval:
            for i in range(self._nb_total_batches):
                ...
                while len(self._circ_buf_feat) < self._feature_batch_seq_len:
                    ...
                    if deepwave_feats.shape[1] < 100:
                        pad_back = 100 - deepwave_feats.shape[1]
                        deepwave_feats = np.pad(deepwave_feats, [(0, 0), (0, pad_back), (0, 0), (0, 0)], mode="constant", constant_values=0.0)
                    # print("after padding", deepwave_feats.shape)
                    # deepwave_feats = deepwave_feats * 0
                    for row_cnt, row in enumerate(deepwave_feats):
            ...
            # print("Total number of batches", self._nb_total_batches)
            for i in range(self._nb_total_batches):
                # load feat and label to circular buffer. Always maintain atleast one batch worth feat and label in the
                # circular buffer. If not keep refilling it.
                while len(self._circ_buf_feat) < self._feature_batch_seq_len:
                    # print("Len list", len(self._filenames_list), file_cnt)
                    # print("Len")
                    temp_feat = np.load(os.path.join(self._feat_dir, self._filenames_list[file_cnt]))
                    temp_label = np.load(os.path.join(self._label_dir, self._filenames_list[file_cnt]))
                    temp_deepwave_feat = []
                    # filename_feat = self._filenames_list[file_cnt].replace("foa", "16bit")
                    deepwave_feats = np.load(f"/scratch/ssd1/feas-upsamp-starrss2023/mic_dev_dw/{self._filenames_list[file_cnt]}")
                    if deepwave_feats.shape[1] < 100:
                        pad_back = 100 - deepwave_feats.shape[1]
                        deepwave_feats = np.pad(deepwave_feats, [(0, 0), (0, pad_back), (0, 0), (0, 0)], mode="constant", constant_values=0.0)
                    deepwave_feats = deepwave_feats.transpose(1, 0, 2, 3)
                    if not self._per_file: 
                        ...
                        temp_deepwave_feat = deepwave_feats[:temp_mul*self._label_seq_len,:,:,:] 
                    else:
                        temp_deepwave_feat = deepwave_feats
                        temp_feat = temp_deepwave_feat
                    for f_row in temp_deepwave_feat:
                    ...
                    if self._per_file:
                        ...
                        extra_feat = np.zeros((feat_extra_frames, temp_feat.shape[1], temp_feat.shape[2], temp_feat.shape[3])) * 1e-6
                ...
                # Read one batch size from the circular buffer
                feat = np.zeros((self._feature_batch_seq_len, 16, 4, 4), dtype=np.complex128) # self._nb_mel_bins * self._nb_ch)) #self._feature_batch_seq_len, self._nb_mel_bins * self._nb_ch))
                # print("Feature seq len", feat.shape)
                for j in range(self._feature_batch_seq_len):
                    feat[j, :, :, :] = self._circ_buf_feat.popleft() # (9, 32, 32)
                    # feat[j, :] = self._circ_buf_feat.popleft()
                # feat = np.reshape(feat, (self._feature_batch_seq_len, self._nb_ch, self._nb_mel_bins)) # (32000, 7, 382)
                ...
                # Split to sequences
                feat = self._split_in_seqs(feat, self._feature_seq_len)
                # print("before split", feat.shape)
                feat = np.transpose(feat, (2, 0, 1, 3, 4)) # (128, 9, 50, 4, 4), original feat shape: (128, 7, 250, 382)
                # print("Feature after split shape", feat.shape)
                feat = np.reshape(feat, (16, -1, 4, 4)) # (freq_bands, Nsamps[batches x T], VisX, VisY)
