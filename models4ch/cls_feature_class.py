...
class FeatureClass:
        ...
        for sub_folder in os.listdir(self._aud_dir):
            ...
            for file_cnt, file_name in enumerate(os.listdir(loc_aud_folder)):
                ...
                if wav_filename == ".wav" or "._" in wav_filename:
                    continue
                # if file_name.split('.')[0] not in self._filewise_frames:
                #     continue
    ...
    def _load_audio(self, audio_path):
        # print("audio path reading", audio_path)
        fs, audio = wav.read(audio_path)
        if audio.shape[0] < fs*10:
            pad_back = fs*10 - audio.shape[0]
            audio = np.pad(audio, [(0, pad_back), (0,0)], mode="constant", constant_values=0.0)
        audio = audio[:, :self._nb_channels] / 32768.0 + self._eps
        return audio, fs

    def _load_audio_deepwave(self, audio_path):
        print("audio path reading", audio_path)
        fs, audio = wav.read(audio_path)
        print("audio shape -- fs", audio.shape, fs)
        if audio.shape[0] < fs*10:
            pad_back = fs*10 - audio.shape[0]
            audio = np.pad(audio, [(0, pad_back), (0, 0)], mode="constant", constant_values=0.0)
        # audio = audio[:, [5,9,25,21]] # / 32768.0 + self._eps
        print(f"Loaded {audio.shape[1]}ch audio")
    ...
    def load_output_format_file(self, _output_format_file):
        ...
        for _line in _fid:
            ...
            if len(_words) == 5: #polar coordinates 
                ...
                _output_dict[_frame_ind].append([int(_words[1]), int(_words[1]), int(_words[2]), float(_words[3])])
                _output_dict = self.convert_output_format_polar_to_cartesian(_output_dict)
        _fid.close()
        return _output_dict
